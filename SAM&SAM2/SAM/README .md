...
The complete workflow for using this project consists of several steps:

![origin_picture](CAluWEgwPX.JPG)

### 1. Extract Masks

First, use `test.py` to extract masks from your images:
...
This step will process your input images and generate corresponding mask files.

### 2. Consolidate Images

Next, use `photo_cp.py` to collect all images from different folders into a single directory:

```bash
poetry run python photo_cp.py
```

This script will:

- Scan through multiple source folders
- Copy all relevant images to a designated output folder
- Maintain proper organization for training

### 3. Standardize Image Format

Run `2png.py` to convert all images to a consistent format:

```bash
poetry run python 2png.py
```

This step ensures:

- All images are in PNG format
- Consistent image properties for training
- Proper handling of different input formats

### 4. Training

After preparing the data, start the training process:

```bash
poetry run python train.py
```

Make sure to configure your training parameters in `config.yaml` before starting.

### 5. Validation

Finally, use `app.py` to validate your trained model:

```bash
poetry run python app.py
```

![test_picture](whole_test.png)

This will launch an interface where you can:

- Test your trained model
- Visualize segmentation results
- Compare with original SAM performance
